...
<p class = "x">
	Please enter the text input</br>
</p>

<textarea rows="10" cols="150" name="ta"><?php
	...
	if(isset($_GET['execute']) && isset($_GET['ta'])&& $_GET['ta']!=''){
		...
		$sentence = $_GET['ta'];
		file_put_contents('InputTextFile.txt',$sentence );
		...
		$filename = 'InputTextFile.txt';
	}
	
	$command = "java -Xmx1g -jar F:\zwalaa\NLP-Gen\SentNEntitiesLocal.jar $filename $filename $filename 2>&1";
	exec($command,$out);
	print_r($out);
?>
